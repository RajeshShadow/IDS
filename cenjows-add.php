...
if($_REQUEST['submitForm']=='yes')
{
  ...
  $Image=new SimpleImage();
    if($_FILES['photo']['size']>0 && $_FILES['photo']['error']=='' && substr_count($_FILES["photo"]["name"], '.') == 1)
    {
      if(in_array($file_extension, $allowed_image_extension)){
        ...
        move_uploaded_file($_FILES['photo']['tmp_name'],"upload_images/doctrine/".$pdf);
            ...
            echo "<script type='text/javascript'>alert('Document Format should be pdf,docx,xml!');window.location=\"cenjows-list.php\";</script>";
        }
	...
	if($mid=='' )
	{
	   ...
	   $sql="INSERT INTO cenjows(title,pdf,author,posted_on,status,link) VALUES('$name','$pdf','$author','$posted',1,'cenjows-paper.php')";
	  $qry=mysqli_query($obj,$sql);
     echo "<script type='text/javascript'>alert('Record added sucessfully');window.location=\"cenjows-list.php\";</script>";
	
    }
        else{
        echo "<script type='text/javascript'>alert('Record Not Added');window.location=\"cenjows-list.php\";</script>";
        }
    }
     if($mid !='')
	{
        
       $sql="update cenjows set title='$name', posted_on='$posted',author='$author' ,status=1";   
    ...
    if($qry==true)
    {
        ...
        echo"<script>window.open('cenjows-list.php','_self')</script>";
    }
    else
        {
...
if($mid!='')
{
	$sql= "SELECT * FROM cenjows where id='$mid'";
	$qry= mysqli_query($obj,$sql);
...
<section class="main">
  ...
  <input type="hidden" name="id" value="<?php echo $_REQUEST['id'];?>" />
      <div class="heading">
      <div class="heading-left">Add Cenjows Paper</div>
